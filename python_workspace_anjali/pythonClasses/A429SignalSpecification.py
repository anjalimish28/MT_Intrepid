...
# -------------------------------------------------------------------
# Automatically generated class from ecore model!! A429SignalSpecification.
# Don't change here, change in source ecore model
# Code generator by Philipp Chrysalidis in 2025
# ======================================================================

class A429SignalSpecification(SuperTrackingType, SuperIdentifierType):
  def __init__(self):
    #attributes
  ...
  def to_xml(self, reference_name = None):
    ...
    if not reference_name == "arinc429" and not reference_name.lower() == "a429signalspecification":
      element.set("xsi:type", "de.ucof.model.communication.arinc429:A429SignalSpecification")  
    ...
    if self.codingType is not None:
      element.set("codingType", str(self.codingType))  
    if self.fieldLength is not None:
      element.set("fieldLength", str(self.fieldLength))  
    # Convert children to nested XML elements
    return element

  def from_xml(self, xml_element):
    # Instantiate an object of type A429SignalSpecification
    obj = A429SignalSpecification()  
    ...
    if "codingType" in xml_element.attrib:
        obj.codingType = xml_element.get("codingType")
    if "fieldLength" in xml_element.attrib:
        obj.fieldLength = xml_element.get("fieldLength")
