...
# -------------------------------------------------------------------
# Automatically generated class from ecore model!! A825Message.
# Don't change here, change in source ecore model
# Code generator by Philipp Chrysalidis in 2025
# ======================================================================

class A825Message(SuperMessageType):
  def __init__(self):
    #attributes
    self.id = None
    self.name = None
  ...
  def to_xml(self, reference_name = None):
    ...
    if not reference_name == "arinc825" and not reference_name.lower() == "a825message":
      element.set("xsi:type", "de.ucof.model.communication.arinc825:A825Message")  
    ...
    if self.dLC is not None:
      element.set("dLC", str(self.dLC))  
    if self.canID is not None:
      element.set("canID", str(self.canID))  
    if self.canFD is not None:
      element.set("canFD", str(self.canFD))  
    # Convert children to nested XML elements
    return element

  def from_xml(self, xml_element):
    # Instantiate an object of type A825Message
    obj = A825Message()  
    ...
    if "dLC" in xml_element.attrib:
        obj.dLC = xml_element.get("dLC")
    if "canID" in xml_element.attrib:
        obj.canID = xml_element.get("canID")
    if "canFD" in xml_element.attrib:
        obj.canFD = xml_element.get("canFD")
