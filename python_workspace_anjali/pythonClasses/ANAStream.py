...
from ANAPacketStrategy import ANAPacketStrategy


# -------------------------------------------------------------------
# Automatically generated class from ecore model!! ANAStream.
# Don't change here, change in source ecore model
# Code generator by Philipp Chrysalidis in 2025
# ======================================================================

class ANAStream(SuperStreamType):
  def __init__(self):
    #attributes
    self.id = None
    self.name = None
    ...
    self.anaPacketStrategy = None  #Initialize as a single child (None by default)
  ...
  def to_xml(self, reference_name = None):
    ...
    if not reference_name == "streamTypes" and not reference_name.lower() == "anastream":
      element.set("xsi:type", "de.ucof.model.communication.configECIC.streamTypes:ANAStream")  
  ...
  # Single child
    if self.anaPacketStrategy is not None:
      element.append(self.anaPacketStrategy.to_xml("anaPacketStrategy"))  
    return element

  def from_xml(self, xml_element):
    # Instantiate an object of type ANAStream
    obj = ANAStream()  
    ...
    # Reference with a single value: find the child element and set the field if it exists
    child_element = xml_element.find("anaPacketStrategy")
    if child_element is not None:  
      xsi_type = child_element.get("{http://www.w3.org/2001/XMLSchema-instance}type")
      if (not xsi_type) or (xsi_type.split(":", 1)[ 1 ].lower() == "anapacketstrategy"):
        child = ANAPacketStrategy.from_xml(self, child_element)
        child.parent = obj
        obj.anaPacketStrategy = child        
