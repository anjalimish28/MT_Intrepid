...
# -------------------------------------------------------------------
# Automatically generated class from ecore model!! CanMailboxAddressFilter.
# Don't change here, change in source ecore model
# Code generator by Philipp Chrysalidis in 2025
# ======================================================================

class CanMailboxAddressFilter(SuperIdentifierType, SuperTrackingType):
  def __init__(self):
    #attributes
    self.id = None
    self.name = None
  ...
  def to_xml(self, reference_name = None):
    ...
    if not reference_name == "intrepidgateway" and not reference_name.lower() == "canmailboxaddressfilter":
      element.set("xsi:type", "de.ucof.model.targets.intrepidgateway:CanMailboxAddressFilter")  
    ...
    if self.addressSymbol is not None:
      element.set("addressSymbol", str(self.addressSymbol))  
    if self.addressValue is not None:
      element.set("addressValue", str(self.addressValue))  
    # Convert children to nested XML elements
    return element

  def from_xml(self, xml_element):
    # Instantiate an object of type CanMailboxAddressFilter
    obj = CanMailboxAddressFilter()  
    ...
    if "addressSymbol" in xml_element.attrib:
        obj.addressSymbol = xml_element.get("addressSymbol")
    if "addressValue" in xml_element.attrib:
        obj.addressValue = xml_element.get("addressValue")
