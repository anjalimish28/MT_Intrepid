...
from ED247Gateway import ED247Gateway

from GenericGateway import GenericGateway

# -------------------------------------------------------------------
# Automatically generated class from ecore model!! CommunicationDevices.
# Don't change here, change in source ecore model
# Code generator by Philipp Chrysalidis in 2025
# ======================================================================

class CommunicationDevices(SuperSetType):
  def __init__(self):
    #attributes
    self.id = None
    self.name = None
    ...
    self.communicationDevice_list = list()  #Initialize as an empty list for multiple children
    self.parent = None #default is no parent, but can be set for every element
	
    #Check if it's a collection (i.e., upperBound <> 1)
  def add_communicationDevice(self, communicationDevice): #function for adding child
    self.communicationDevice_list.append(communicationDevice)
    communicationDevice.parent = self #link child to parent

  def to_xml(self, reference_name = None):
    element = ET.Element(reference_name)
    if not reference_name == "comDevices" and not reference_name.lower() == "communicationdevices":
      element.set("xsi:type", "de.ucof.model.communication.comDevices:CommunicationDevices")  
  ...
  # Multiple children
    for child in self.communicationDevice_list:  
      element.append(child.to_xml("communicationDevice"))  
    return element

  def from_xml(self, xml_element):
    # Instantiate an object of type CommunicationDevices
    obj = CommunicationDevices()  
    ...
    obj.communicationDevice_list = list()
    for child_element in xml_element.findall("communicationDevice"):
      xsi_type = child_element.get("{http://www.w3.org/2001/XMLSchema-instance}type")
      if not xsi_type:
        child = SuperCommunicationDeviceType.from_xml(self, child_element)
        child.parent = obj
        obj.communicationDevice_list.append(child)
      elif xsi_type.split(":", 1)[ 1 ].lower() == "ed247gateway":
        child = ED247Gateway.from_xml(self, child_element)
        child.parent = obj
        obj.communicationDevice_list.append(child)
      
      elif xsi_type.split(":", 1)[ 1 ].lower() == "genericgateway":
        child = GenericGateway.from_xml(self, child_element)
        child.parent = obj
        obj.communicationDevice_list.append(child)
    return obj
