...
# -------------------------------------------------------------------
# Automatically generated class from ecore model!! ConnectorGroup.
# Don't change here, change in source ecore model
# Code generator by Philipp Chrysalidis in 2025
# ======================================================================

class ConnectorGroup(SuperIdentifierType, SuperTrackingType):
  def __init__(self):
    #attributes
    self.id = None
    self.name = None
    ...
    self.connector_list = list()  #Initialize as an empty list for multiple children
    self.parent = None #default is no parent, but can be set for every element
	
    #Check if it's a collection (i.e., upperBound <> 1)
  def add_connector(self, connector): #function for adding child
    self.connector_list.append(connector)
    connector.parent = self #link child to parent

  def to_xml(self, reference_name = None):
    element = ET.Element(reference_name)
    if not reference_name == "connectors" and not reference_name.lower() == "connectorgroup":
      element.set("xsi:type", "de.ucof.model.communication.connectors:ConnectorGroup")  
  ...
  # Multiple children
    for child in self.connector_list:  
      element.append(child.to_xml("connector"))  
    return element

  def from_xml(self, xml_element):
    # Instantiate an object of type ConnectorGroup
    obj = ConnectorGroup()  
