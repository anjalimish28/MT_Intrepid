...
from ConnectorGroup import ConnectorGroup


# -------------------------------------------------------------------
# Automatically generated class from ecore model!! Connectors.
# Don't change here, change in source ecore model
# Code generator by Philipp Chrysalidis in 2025
# ======================================================================

class Connectors(SuperTrackingType, SuperIdentifierType):
  def __init__(self):
    #attributes
    ...
    self.connectorGroup_list = list()  #Initialize as an empty list for multiple children
    self.parent = None #default is no parent, but can be set for every element
	
    #Check if it's a collection (i.e., upperBound <> 1)
  def add_connectorGroup(self, connectorGroup): #function for adding child
    self.connectorGroup_list.append(connectorGroup)
    connectorGroup.parent = self #link child to parent

  def to_xml(self, reference_name = None):
    element = ET.Element(reference_name)
    if not reference_name == "connectors" and not reference_name.lower() == "connectors":
      element.set("xsi:type", "de.ucof.model.communication.connectors:Connectors")  
  ...
  # Multiple children
    for child in self.connectorGroup_list:  
      element.append(child.to_xml("connectorGroup"))  
    return element

  def from_xml(self, xml_element):
    # Instantiate an object of type Connectors
    obj = Connectors()  
    ...
    obj.connectorGroup_list = list()
    for child_element in xml_element.findall("connectorGroup"):
      xsi_type = child_element.get("{http://www.w3.org/2001/XMLSchema-instance}type")
      if not xsi_type:
        child = ConnectorGroup.from_xml(self, child_element)
        child.parent = obj
        obj.connectorGroup_list.append(child)
