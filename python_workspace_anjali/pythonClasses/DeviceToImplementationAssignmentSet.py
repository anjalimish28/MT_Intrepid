...
from DeviceToImplementationAssignment import DeviceToImplementationAssignment


# -------------------------------------------------------------------
# Automatically generated class from ecore model!! DeviceToImplementationAssignmentSet.
# Don't change here, change in source ecore model
# Code generator by Philipp Chrysalidis in 2025
# ======================================================================

class DeviceToImplementationAssignmentSet(SuperAssignmentSetType):
  def __init__(self):
    #attributes
    self.id = None
    self.name = None
    ...
    self.deviceToImplementationAssignment_list = list()  #Initialize as an empty list for multiple children
    self.parent = None #default is no parent, but can be set for every element
	
    #Check if it's a collection (i.e., upperBound <> 1)
  def add_deviceToImplementationAssignment(self, deviceToImplementationAssignment): #function for adding child
    self.deviceToImplementationAssignment_list.append(deviceToImplementationAssignment)
    deviceToImplementationAssignment.parent = self #link child to parent

  def to_xml(self, reference_name = None):
    element = ET.Element(reference_name)
    if not reference_name == "assignments" and not reference_name.lower() == "devicetoimplementationassignmentset":
      element.set("xsi:type", "de.ucof.model.assignments:DeviceToImplementationAssignmentSet")  
  ...
  # Multiple children
    for child in self.deviceToImplementationAssignment_list:  
      element.append(child.to_xml("deviceToImplementationAssignment"))  
    return element

  def from_xml(self, xml_element):
    # Instantiate an object of type DeviceToImplementationAssignmentSet
    obj = DeviceToImplementationAssignmentSet()  
    ...
    obj.deviceToImplementationAssignment_list = list()
    for child_element in xml_element.findall("deviceToImplementationAssignment"):
      xsi_type = child_element.get("{http://www.w3.org/2001/XMLSchema-instance}type")
      if not xsi_type:
        child = DeviceToImplementationAssignment.from_xml(self, child_element)
        child.parent = obj
        obj.deviceToImplementationAssignment_list.append(child)
