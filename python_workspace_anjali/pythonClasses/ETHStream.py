...
from ETHPacketStrategy import ETHPacketStrategy


# -------------------------------------------------------------------
# Automatically generated class from ecore model!! ETHStream.
# Don't change here, change in source ecore model
# Code generator by Philipp Chrysalidis in 2025
# ======================================================================

class ETHStream(SuperStreamType):
  def __init__(self):
    #attributes
    self.id = None
    self.name = None
    ...
    self.ethPacketStrategy = None  #Initialize as a single child (None by default)
  ...
  def to_xml(self, reference_name = None):
    ...
    if not reference_name == "streamTypes" and not reference_name.lower() == "ethstream":
      element.set("xsi:type", "de.ucof.model.communication.configECIC.streamTypes:ETHStream")  
  ...
  # Single child
    if self.ethPacketStrategy is not None:
      element.append(self.ethPacketStrategy.to_xml("ethPacketStrategy"))  
    return element

  def from_xml(self, xml_element):
    # Instantiate an object of type ETHStream
    obj = ETHStream()  
    ...
    # Reference with a single value: find the child element and set the field if it exists
    child_element = xml_element.find("ethPacketStrategy")
    if child_element is not None:  
      xsi_type = child_element.get("{http://www.w3.org/2001/XMLSchema-instance}type")
      if (not xsi_type) or (xsi_type.split(":", 1)[ 1 ].lower() == "ethpacketstrategy"):
        child = ETHPacketStrategy.from_xml(self, child_element)
        child.parent = obj
        obj.ethPacketStrategy = child        
