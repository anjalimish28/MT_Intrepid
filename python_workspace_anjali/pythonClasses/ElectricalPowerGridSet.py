...
from intrepidElectricalGrid import intrepidElectricalGrid

# -------------------------------------------------------------------
# Automatically generated class from ecore model!! ElectricalPowerGridSet.
# Don't change here, change in source ecore model
# Code generator by Philipp Chrysalidis in 2025
# ======================================================================

class ElectricalPowerGridSet(SuperIdentifierType, SuperTrackingType):
  def __init__(self):
    #attributes
    self.id = None
    self.name = None
    ...
    self.electricalGrid_list = list()  #Initialize as an empty list for multiple children
    self.parent = None #default is no parent, but can be set for every element
	
    #Check if it's a collection (i.e., upperBound <> 1)
  def add_electricalGrid(self, electricalGrid): #function for adding child
    self.electricalGrid_list.append(electricalGrid)
    electricalGrid.parent = self #link child to parent

  def to_xml(self, reference_name = None):
    element = ET.Element(reference_name)
    if not reference_name == "powergrid" and not reference_name.lower() == "electricalpowergridset":
      element.set("xsi:type", "de.ucof.model.targets.powergrid:ElectricalPowerGridSet")  
  ...
  # Multiple children
    for child in self.electricalGrid_list:  
      element.append(child.to_xml("electricalGrid"))  
    return element

  def from_xml(self, xml_element):
    # Instantiate an object of type ElectricalPowerGridSet
    obj = ElectricalPowerGridSet()  
    ...
    obj.electricalGrid_list = list()
    for child_element in xml_element.findall("electricalGrid"):
      xsi_type = child_element.get("{http://www.w3.org/2001/XMLSchema-instance}type")
      if not xsi_type:
        child = SuperElectricalGridType.from_xml(self, child_element)
        child.parent = obj
        obj.electricalGrid_list.append(child)
      elif xsi_type.split(":", 1)[ 1 ].lower() == "intrepidelectricalgrid":
        child = intrepidElectricalGrid.from_xml(self, child_element)
        child.parent = obj
        obj.electricalGrid_list.append(child)
    return obj
