...
# -------------------------------------------------------------------
# Automatically generated class from ecore model!! ErrorInfoModule.
# Don't change here, change in source ecore model
# Code generator by Philipp Chrysalidis in 2025
# ======================================================================

class ErrorInfoModule(ErrorInfoSuper):
  def __init__(self):
    #attributes
    self.id = None
    self.name = None
    ...
    #children (references)
    self.moduleRecoveryAction = None  #Initialize as a single child (None by default)
  ...
  def to_xml(self, reference_name = None):
    ...
    if not reference_name == "healthmanagement" and not reference_name.lower() == "errorinfomodule":
      element.set("xsi:type", "de.ucof.model.healthmanagement:ErrorInfoModule")  
  ...
  # Single child
    if self.moduleRecoveryAction is not None:
      element.append(self.moduleRecoveryAction.to_xml("moduleRecoveryAction"))  
    return element

  def from_xml(self, xml_element):
    # Instantiate an object of type ErrorInfoModule
    obj = ErrorInfoModule()  
    ...
    child_element = xml_element.find("moduleRecoveryAction")
    if child_element is not None:  
      xsi_type = child_element.get("{http://www.w3.org/2001/XMLSchema-instance}type")
      if (not xsi_type) or (xsi_type.split(":", 1)[ 1 ].lower() == "modulerecoveryaction"):
        child = ModuleRecoveryAction.from_xml(self, child_element)
        child.parent = obj
        obj.moduleRecoveryAction = child        
