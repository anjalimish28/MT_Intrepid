...
from MultiPartitionTables import MultiPartitionTables


# -------------------------------------------------------------------
# Automatically generated class from ecore model!! HealthManagement.
# Don't change here, change in source ecore model
# Code generator by Philipp Chrysalidis in 2025
# ======================================================================

class HealthManagement(SuperIdentifierType, SuperTrackingType):
  def __init__(self):
    #attributes
    self.id = None
    self.name = None
    ...
    self.version = None
    #children (references)
    self.moduleTable = None  #Initialize as a single child (None by default)
    self.multiPartitionTables = None  #Initialize as a single child (None by default)
  ...
  def to_xml(self, reference_name = None):
    ...
    if not reference_name == "healthmanagement" and not reference_name.lower() == "healthmanagement":
      element.set("xsi:type", "de.ucof.model.healthmanagement:HealthManagement")  
  ...
  # Single child
    if self.moduleTable is not None:
      element.append(self.moduleTable.to_xml("moduleTable"))  
  # Single child
    if self.multiPartitionTables is not None:
      element.append(self.multiPartitionTables.to_xml("multiPartitionTables"))  
    return element

  def from_xml(self, xml_element):
    # Instantiate an object of type HealthManagement
    obj = HealthManagement()  
    ...
    child_element = xml_element.find("moduleTable")
    if child_element is not None:  
      xsi_type = child_element.get("{http://www.w3.org/2001/XMLSchema-instance}type")
      if (not xsi_type) or (xsi_type.split(":", 1)[ 1 ].lower() == "moduletable"):
        child = ModuleTable.from_xml(self, child_element)
        child.parent = obj
        obj.moduleTable = child        
      
    # Reference with a single value: find the child element and set the field if it exists
    child_element = xml_element.find("multiPartitionTables")
    if child_element is not None:  
      xsi_type = child_element.get("{http://www.w3.org/2001/XMLSchema-instance}type")
      if (not xsi_type) or (xsi_type.split(":", 1)[ 1 ].lower() == "multipartitiontables"):
        child = MultiPartitionTables.from_xml(self, child_element)
        child.parent = obj
        obj.multiPartitionTables = child        
