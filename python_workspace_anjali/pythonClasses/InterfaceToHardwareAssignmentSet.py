...
from InterfaceToHardwareAssignment import InterfaceToHardwareAssignment


# -------------------------------------------------------------------
# Automatically generated class from ecore model!! InterfaceToHardwareAssignmentSet.
# Don't change here, change in source ecore model
# Code generator by Philipp Chrysalidis in 2025
# ======================================================================

class InterfaceToHardwareAssignmentSet(SuperAssignmentSetType):
  def __init__(self):
    #attributes
    self.id = None
    self.name = None
    ...
    self.interfaceToHardwareAssignment_list = list()  #Initialize as an empty list for multiple children
    self.parent = None #default is no parent, but can be set for every element
	
    #Check if it's a collection (i.e., upperBound <> 1)
  def add_interfaceToHardwareAssignment(self, interfaceToHardwareAssignment): #function for adding child
    self.interfaceToHardwareAssignment_list.append(interfaceToHardwareAssignment)
    interfaceToHardwareAssignment.parent = self #link child to parent

  def to_xml(self, reference_name = None):
    element = ET.Element(reference_name)
    if not reference_name == "assignments" and not reference_name.lower() == "interfacetohardwareassignmentset":
      element.set("xsi:type", "de.ucof.model.assignments:InterfaceToHardwareAssignmentSet")  
  ...
  # Multiple children
    for child in self.interfaceToHardwareAssignment_list:  
      element.append(child.to_xml("interfaceToHardwareAssignment"))  
    return element

  def from_xml(self, xml_element):
    # Instantiate an object of type InterfaceToHardwareAssignmentSet
    obj = InterfaceToHardwareAssignmentSet()  
    ...
    obj.interfaceToHardwareAssignment_list = list()
    for child_element in xml_element.findall("interfaceToHardwareAssignment"):
      xsi_type = child_element.get("{http://www.w3.org/2001/XMLSchema-instance}type")
      if not xsi_type:
        child = InterfaceToHardwareAssignment.from_xml(self, child_element)
        child.parent = obj
        obj.interfaceToHardwareAssignment_list.append(child)
