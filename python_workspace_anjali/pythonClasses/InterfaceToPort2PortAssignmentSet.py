...
from InterfaceToPort2PortAssignment import InterfaceToPort2PortAssignment


# -------------------------------------------------------------------
# Automatically generated class from ecore model!! InterfaceToPort2PortAssignmentSet.
# Don't change here, change in source ecore model
# Code generator by Philipp Chrysalidis in 2025
# ======================================================================

class InterfaceToPort2PortAssignmentSet(SuperAssignmentSetType):
  def __init__(self):
    #attributes
    self.id = None
    self.name = None
    ...
    self.interfaceToPortAssignment_list = list()  #Initialize as an empty list for multiple children
    self.parent = None #default is no parent, but can be set for every element
	
    #Check if it's a collection (i.e., upperBound <> 1)
  def add_interfaceToPortAssignment(self, interfaceToPortAssignment): #function for adding child
    self.interfaceToPortAssignment_list.append(interfaceToPortAssignment)
    interfaceToPortAssignment.parent = self #link child to parent

  def to_xml(self, reference_name = None):
    element = ET.Element(reference_name)
    if not reference_name == "assignments" and not reference_name.lower() == "interfacetoport2portassignmentset":
      element.set("xsi:type", "de.ucof.model.assignments:InterfaceToPort2PortAssignmentSet")  
  ...
  # Multiple children
    for child in self.interfaceToPortAssignment_list:  
      element.append(child.to_xml("interfaceToPortAssignment"))  
    return element

  def from_xml(self, xml_element):
    # Instantiate an object of type InterfaceToPort2PortAssignmentSet
    obj = InterfaceToPort2PortAssignmentSet()  
    ...
    obj.interfaceToPortAssignment_list = list()
    for child_element in xml_element.findall("interfaceToPortAssignment"):
      xsi_type = child_element.get("{http://www.w3.org/2001/XMLSchema-instance}type")
      if not xsi_type:
        child = InterfaceToPort2PortAssignment.from_xml(self, child_element)
        child.parent = obj
        obj.interfaceToPortAssignment_list.append(child)
