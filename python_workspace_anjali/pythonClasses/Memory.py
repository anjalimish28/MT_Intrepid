...
from MemoryRequirement import MemoryRequirement


# -------------------------------------------------------------------
# Automatically generated class from ecore model!! Memory.
# Don't change here, change in source ecore model
# Code generator by Philipp Chrysalidis in 2025
# ======================================================================

class Memory(SuperIdentifierType, SuperTrackingType):
  def __init__(self):
    #attributes
    self.id = None
    self.name = None
    ...
    self.memoryRequirements_list = list()  #Initialize as an empty list for multiple children
    self.parent = None #default is no parent, but can be set for every element
	
    #Check if it's a collection (i.e., upperBound <> 1)
  def add_memoryRequirements(self, memoryRequirements): #function for adding child
    self.memoryRequirements_list.append(memoryRequirements)
    memoryRequirements.parent = self #link child to parent

  def to_xml(self, reference_name = None):
    element = ET.Element(reference_name)
    if not reference_name == "etypes" and not reference_name.lower() == "memory":
      element.set("xsi:type", "de.ucof.model.common.etypes:Memory")  
  ...
  # Multiple children
    for child in self.memoryRequirements_list:  
      element.append(child.to_xml("memoryRequirements"))  
    return element

  def from_xml(self, xml_element):
    # Instantiate an object of type Memory
    obj = Memory()  
    ...
    obj.memoryRequirements_list = list()
    for child_element in xml_element.findall("memoryRequirements"):
      xsi_type = child_element.get("{http://www.w3.org/2001/XMLSchema-instance}type")
      if not xsi_type:
        child = MemoryRequirement.from_xml(self, child_element)
        child.parent = obj
        obj.memoryRequirements_list.append(child)
