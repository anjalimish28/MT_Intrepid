...
from MessageToTargetAssignment import MessageToTargetAssignment


# -------------------------------------------------------------------
# Automatically generated class from ecore model!! MessageToTargetAssignmentSet.
# Don't change here, change in source ecore model
# Code generator by Philipp Chrysalidis in 2025
# ======================================================================

class MessageToTargetAssignmentSet(SuperAssignmentSetType):
  def __init__(self):
    #attributes
    self.id = None
    self.name = None
    ...
    self.messageToTargetAssignment_list = list()  #Initialize as an empty list for multiple children
    self.parent = None #default is no parent, but can be set for every element
	
    #Check if it's a collection (i.e., upperBound <> 1)
  def add_messageToTargetAssignment(self, messageToTargetAssignment): #function for adding child
    self.messageToTargetAssignment_list.append(messageToTargetAssignment)
    messageToTargetAssignment.parent = self #link child to parent

  def to_xml(self, reference_name = None):
    element = ET.Element(reference_name)
    if not reference_name == "database" and not reference_name.lower() == "messagetotargetassignmentset":
      element.set("xsi:type", "de.ucof.model.assignments.database:MessageToTargetAssignmentSet")  
  ...
  # Multiple children
    for child in self.messageToTargetAssignment_list:  
      element.append(child.to_xml("messageToTargetAssignment"))  
    return element

  def from_xml(self, xml_element):
    # Instantiate an object of type MessageToTargetAssignmentSet
    obj = MessageToTargetAssignmentSet()  
    ...
    obj.messageToTargetAssignment_list = list()
    for child_element in xml_element.findall("messageToTargetAssignment"):
      xsi_type = child_element.get("{http://www.w3.org/2001/XMLSchema-instance}type")
      if not xsi_type:
        child = MessageToTargetAssignment.from_xml(self, child_element)
        child.parent = obj
        obj.messageToTargetAssignment_list.append(child)
