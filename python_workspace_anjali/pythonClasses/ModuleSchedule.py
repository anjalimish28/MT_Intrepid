...
from PartitionTimeWindow import PartitionTimeWindow


# -------------------------------------------------------------------
# Automatically generated class from ecore model!! ModuleSchedule.
# Don't change here, change in source ecore model
# Code generator by Philipp Chrysalidis in 2025
# ======================================================================

class ModuleSchedule(SuperIdentifierType, SuperTrackingType):
  def __init__(self):
    #attributes
    self.id = None
    self.name = None
    ...
    self.partitionTimeWindow_list = list()  #Initialize as an empty list for multiple children
    self.parent = None #default is no parent, but can be set for every element
	
    #Check if it's a collection (i.e., upperBound <> 1)
  def add_partitionTimeWindow(self, partitionTimeWindow): #function for adding child
    self.partitionTimeWindow_list.append(partitionTimeWindow)
    partitionTimeWindow.parent = self #link child to parent

  def to_xml(self, reference_name = None):
    element = ET.Element(reference_name)
    if not reference_name == "a653components" and not reference_name.lower() == "moduleschedule":
      element.set("xsi:type", "de.ucof.model.a653components:ModuleSchedule")  
  ...
  # Multiple children
    for child in self.partitionTimeWindow_list:  
      element.append(child.to_xml("partitionTimeWindow"))  
    return element

  def from_xml(self, xml_element):
    # Instantiate an object of type ModuleSchedule
    obj = ModuleSchedule()  
    ...
    obj.partitionTimeWindow_list = list()
    for child_element in xml_element.findall("partitionTimeWindow"):
      xsi_type = child_element.get("{http://www.w3.org/2001/XMLSchema-instance}type")
      if not xsi_type:
        child = PartitionTimeWindow.from_xml(self, child_element)
        child.parent = obj
        obj.partitionTimeWindow_list.append(child)
