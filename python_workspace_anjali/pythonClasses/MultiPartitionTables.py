...
# -------------------------------------------------------------------
# Automatically generated class from ecore model!! MultiPartitionTables.
# Don't change here, change in source ecore model
# Code generator by Philipp Chrysalidis in 2025
# ======================================================================

class MultiPartitionTables(SuperIdentifierType, SuperTrackingType):
  def __init__(self):
    #attributes
    self.id = None
    self.name = None
    ...
    self.multiPartitionTable_list = list()  #Initialize as an empty list for multiple children
    self.parent = None #default is no parent, but can be set for every element
	
    #Check if it's a collection (i.e., upperBound <> 1)
  def add_multiPartitionTable(self, multiPartitionTable): #function for adding child
    self.multiPartitionTable_list.append(multiPartitionTable)
    multiPartitionTable.parent = self #link child to parent

  def to_xml(self, reference_name = None):
    element = ET.Element(reference_name)
    if not reference_name == "healthmanagement" and not reference_name.lower() == "multipartitiontables":
      element.set("xsi:type", "de.ucof.model.healthmanagement:MultiPartitionTables")  
  ...
  # Multiple children
    for child in self.multiPartitionTable_list:  
      element.append(child.to_xml("multiPartitionTable"))  
    return element

  def from_xml(self, xml_element):
    # Instantiate an object of type MultiPartitionTables
    obj = MultiPartitionTables()  
    ...
    obj.multiPartitionTable_list = list()
    for child_element in xml_element.findall("multiPartitionTable"):
      ...
      if not xsi_type:
        ...
        child.parent = obj
        obj.multiPartitionTable_list.append(child)
