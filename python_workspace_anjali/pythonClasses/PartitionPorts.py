...
from SamplingPort import SamplingPort

from QueuingPort import QueuingPort

# -------------------------------------------------------------------
# Automatically generated class from ecore model!! PartitionPorts.
# Don't change here, change in source ecore model
# Code generator by Philipp Chrysalidis in 2025
# ======================================================================

class PartitionPorts(SuperTrackingType, SuperIdentifierType):
  def __init__(self):
    #attributes
  ...
  def to_xml(self, reference_name = None):
    ...
    if not reference_name == "ports" and not reference_name.lower() == "partitionports":
      element.set("xsi:type", "de.ucof.model.devices.ports:PartitionPorts")  
  ...
  def from_xml(self, xml_element):
    # Instantiate an object of type PartitionPorts
    obj = PartitionPorts()  
    ...
    for child_element in xml_element.findall("port"):
      ...
      if not xsi_type:
        child = SuperInterCommunicationPortType.from_xml(self, child_element)
        child.parent = obj
        obj.port_list.append(child)
      elif xsi_type.split(":", 1)[ 1 ].lower() == "samplingport":
        child = SamplingPort.from_xml(self, child_element)
        child.parent = obj
        obj.port_list.append(child)
      
      elif xsi_type.split(":", 1)[ 1 ].lower() == "queuingport":
        child = QueuingPort.from_xml(self, child_element)
        child.parent = obj
        obj.port_list.append(child)
    return obj
