...
# -------------------------------------------------------------------
# Automatically generated class from ecore model!! PartitionTimeWindow.
# Don't change here, change in source ecore model
# Code generator by Philipp Chrysalidis in 2025
# ======================================================================

class PartitionTimeWindow(SuperIdentifierType, SuperTrackingType):
  def __init__(self):
    #attributes
    self.id = None
    self.name = None
    ...
    self.duration = None
    self.offset = None
    self.periodicProcessingStart = None
    self.partitionOrPortID = None
    #children (references)
  ...
  def to_xml(self, reference_name = None):
    ...
    if not reference_name == "a653components" and not reference_name.lower() == "partitiontimewindow":
      element.set("xsi:type", "de.ucof.model.a653components:PartitionTimeWindow")  
    ...
    if self.periodicProcessingStart is not None:
      element.set("periodicProcessingStart", str(self.periodicProcessingStart))  
    if self.partitionOrPortID is not None:
      element.set("partitionOrPortID", str(self.partitionOrPortID))  
    # Convert children to nested XML elements
    return element

  def from_xml(self, xml_element):
    # Instantiate an object of type PartitionTimeWindow
    obj = PartitionTimeWindow()  
    ...
    if "periodicProcessingStart" in xml_element.attrib:
        obj.periodicProcessingStart = xml_element.get("periodicProcessingStart")
    if "partitionOrPortID" in xml_element.attrib:
        obj.partitionOrPortID = xml_element.get("partitionOrPortID")
