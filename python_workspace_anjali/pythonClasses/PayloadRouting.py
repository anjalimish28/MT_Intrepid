...
# -------------------------------------------------------------------
# Automatically generated class from ecore model!! PayloadRouting.
# Don't change here, change in source ecore model
# Code generator by Philipp Chrysalidis in 2025
# ======================================================================

class PayloadRouting(SuperIdentifierType, SuperTrackingType):
  def __init__(self):
    #attributes
    self.id = None
    self.name = None
    ...
    self.version = None
    #children (references)
    self.inPayload = None  #Initialize as a single child (None by default)
    self.outPayload = None  #Initialize as a single child (None by default)
  ...
  def to_xml(self, reference_name = None):
    ...
    if not reference_name == "routing" and not reference_name.lower() == "payloadrouting":
      element.set("xsi:type", "de.ucof.model.devices.functions.routing:PayloadRouting")  
  ...
  # Single child
    if self.inPayload is not None:
      element.append(self.inPayload.to_xml("inPayload"))  
  # Single child
    if self.outPayload is not None:
      element.append(self.outPayload.to_xml("outPayload"))  
    return element

  def from_xml(self, xml_element):
    # Instantiate an object of type PayloadRouting
    obj = PayloadRouting()  
    ...
    child_element = xml_element.find("inPayload")
    if child_element is not None:  
      xsi_type = child_element.get("{http://www.w3.org/2001/XMLSchema-instance}type")
      if (not xsi_type) or (xsi_type.split(":", 1)[ 1 ].lower() == "payload"):
        child = Payload.from_xml(self, child_element)
        child.parent = obj
        obj.inPayload = child        
      
    # Reference with a single value: find the child element and set the field if it exists
    child_element = xml_element.find("outPayload")
    if child_element is not None:  
      xsi_type = child_element.get("{http://www.w3.org/2001/XMLSchema-instance}type")
      if (not xsi_type) or (xsi_type.split(":", 1)[ 1 ].lower() == "payload"):
        child = Payload.from_xml(self, child_element)
        child.parent = obj
        obj.outPayload = child        
