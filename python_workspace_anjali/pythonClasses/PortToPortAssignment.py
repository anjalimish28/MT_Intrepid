...
# -------------------------------------------------------------------
# Automatically generated class from ecore model!! PortToPortAssignment.
# Don't change here, change in source ecore model
# Code generator by Philipp Chrysalidis in 2025
# ======================================================================

class PortToPortAssignment(SuperAssignmentType):
  def __init__(self):
    #attributes
    self.id = None
    self.name = None
  ...
  def to_xml(self, reference_name = None):
    ...
    if not reference_name == "assignments" and not reference_name.lower() == "porttoportassignment":
      element.set("xsi:type", "de.ucof.model.assignments:PortToPortAssignment")  
    ...
    if self.transmissionRate is not None:
      element.set("transmissionRate", str(self.transmissionRate))  
    if self.refreshRate is not None:
      element.set("refreshRate", str(self.refreshRate))  
    # Convert children to nested XML elements
    return element

  def from_xml(self, xml_element):
    # Instantiate an object of type PortToPortAssignment
    obj = PortToPortAssignment()  
    ...
    if "transmissionRate" in xml_element.attrib:
        obj.transmissionRate = xml_element.get("transmissionRate")
    if "refreshRate" in xml_element.attrib:
        obj.refreshRate = xml_element.get("refreshRate")
