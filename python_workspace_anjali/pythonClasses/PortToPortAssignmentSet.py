...
from PortToPortAssignment import PortToPortAssignment


# -------------------------------------------------------------------
# Automatically generated class from ecore model!! PortToPortAssignmentSet.
# Don't change here, change in source ecore model
# Code generator by Philipp Chrysalidis in 2025
# ======================================================================

class PortToPortAssignmentSet(SuperAssignmentSetType):
  def __init__(self):
    #attributes
    self.id = None
    self.name = None
    ...
    self.portToPortAssignment_list = list()  #Initialize as an empty list for multiple children
    self.parent = None #default is no parent, but can be set for every element
	
    #Check if it's a collection (i.e., upperBound <> 1)
  def add_portToPortAssignment(self, portToPortAssignment): #function for adding child
    self.portToPortAssignment_list.append(portToPortAssignment)
    portToPortAssignment.parent = self #link child to parent

  def to_xml(self, reference_name = None):
    element = ET.Element(reference_name)
    if not reference_name == "assignments" and not reference_name.lower() == "porttoportassignmentset":
      element.set("xsi:type", "de.ucof.model.assignments:PortToPortAssignmentSet")  
  ...
  # Multiple children
    for child in self.portToPortAssignment_list:  
      element.append(child.to_xml("portToPortAssignment"))  
    return element

  def from_xml(self, xml_element):
    # Instantiate an object of type PortToPortAssignmentSet
    obj = PortToPortAssignmentSet()  
    ...
    obj.portToPortAssignment_list = list()
    for child_element in xml_element.findall("portToPortAssignment"):
      xsi_type = child_element.get("{http://www.w3.org/2001/XMLSchema-instance}type")
      if not xsi_type:
        child = PortToPortAssignment.from_xml(self, child_element)
        child.parent = obj
        obj.portToPortAssignment_list.append(child)
