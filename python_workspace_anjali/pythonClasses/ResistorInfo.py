...
# -------------------------------------------------------------------
# Automatically generated class from ecore model!! ResistorInfo.
# Don't change here, change in source ecore model
# Code generator by Philipp Chrysalidis in 2025
# ======================================================================

class ResistorInfo(SuperAssignmentSpecificationType):
  def __init__(self):
    #attributes
    self.id = None
    self.name = None
    ...
    self.resistanceOhm = None
    self.resistorPartNumber = None
    self.mountLocation = None
    #children (references)
  ...
  def to_xml(self, reference_name = None):
    ...
    if not reference_name == "targets" and not reference_name.lower() == "resistorinfo":
      element.set("xsi:type", "de.ucof.model.targets:ResistorInfo")  
    ...
    if self.resistanceOhm is not None:
      element.set("resistanceOhm", str(self.resistanceOhm))  
    if self.resistorPartNumber is not None:
      element.set("resistorPartNumber", str(self.resistorPartNumber))  
    if self.mountLocation is not None:
      element.set("mountLocation", str(self.mountLocation))  
    # Convert children to nested XML elements
    return element

  def from_xml(self, xml_element):
    # Instantiate an object of type ResistorInfo
    obj = ResistorInfo()  
    ...
    if "resistanceOhm" in xml_element.attrib:
        obj.resistanceOhm = xml_element.get("resistanceOhm")
    if "resistorPartNumber" in xml_element.attrib:
        obj.resistorPartNumber = xml_element.get("resistorPartNumber")
    if "mountLocation" in xml_element.attrib:
        obj.mountLocation = xml_element.get("mountLocation")
