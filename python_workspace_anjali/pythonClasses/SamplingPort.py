...
#import super types
from SuperInterCommunicationPortType import SuperInterCommunicationPortType

#import children classes

# -------------------------------------------------------------------
# Automatically generated class from ecore model!! SamplingPort.
# Don't change here, change in source ecore model
# Code generator by Philipp Chrysalidis in 2025
# ======================================================================

class SamplingPort(SuperInterCommunicationPortType):
  def __init__(self):
    #attributes
    self.id = None
    self.name = None
    ...
    self.refreshPeriodORTransmissionRate = None
    #children (references)
  ...
  def to_xml(self, reference_name = None):
    ...
    if not reference_name == "etypes" and not reference_name.lower() == "samplingport":
      element.set("xsi:type", "de.ucof.model.common.etypes:SamplingPort")  
    ...
    if self.maxMessageSize is not None:
      element.set("maxMessageSize", str(self.maxMessageSize))  
    if self.portDirection is not None:
      element.set("portDirection", str(self.portDirection))  
    if self.refreshPeriodORTransmissionRate is not None:
      element.set("refreshPeriodORTransmissionRate", str(self.refreshPeriodORTransmissionRate))  
    # Convert children to nested XML elements
    return element

  def from_xml(self, xml_element):
    # Instantiate an object of type SamplingPort
    obj = SamplingPort()  
    ...
    if "maxMessageSize" in xml_element.attrib:
        obj.maxMessageSize = xml_element.get("maxMessageSize")
    if "portDirection" in xml_element.attrib:
        obj.portDirection = xml_element.get("portDirection")
    if "refreshPeriodORTransmissionRate" in xml_element.attrib:
        obj.refreshPeriodORTransmissionRate = xml_element.get("refreshPeriodORTransmissionRate")
