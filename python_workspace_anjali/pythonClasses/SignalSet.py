...
# -------------------------------------------------------------------
# Automatically generated class from ecore model!! SignalSet.
# Don't change here, change in source ecore model
# Code generator by Philipp Chrysalidis in 2025
# ======================================================================

class SignalSet(CommunicationSetType):
  def __init__(self):
    #attributes
    self.id = None
    self.name = None
    ...
    self.signal_list = list()  #Initialize as an empty list for multiple children
    self.parent = None #default is no parent, but can be set for every element
	
    #Check if it's a collection (i.e., upperBound <> 1)
  def add_signal(self, signal): #function for adding child
    self.signal_list.append(signal)
    signal.parent = self #link child to parent

  def to_xml(self, reference_name = None):
    element = ET.Element(reference_name)
    if not reference_name == "communication" and not reference_name.lower() == "signalset":
      element.set("xsi:type", "de.ucof.model.communication:SignalSet")  
  ...
  # Multiple children
    for child in self.signal_list:  
      element.append(child.to_xml("signal"))  
    return element

  def from_xml(self, xml_element):
    # Instantiate an object of type SignalSet
    obj = SignalSet()  
    ...
    obj.signal_list = list()
    for child_element in xml_element.findall("signal"):
      xsi_type = child_element.get("{http://www.w3.org/2001/XMLSchema-instance}type")
      if not xsi_type:
        child = Signal.from_xml(self, child_element)
        child.parent = obj
        obj.signal_list.append(child)
