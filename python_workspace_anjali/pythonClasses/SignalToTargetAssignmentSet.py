...
from SignalToTargetAssignment import SignalToTargetAssignment


# -------------------------------------------------------------------
# Automatically generated class from ecore model!! SignalToTargetAssignmentSet.
# Don't change here, change in source ecore model
# Code generator by Philipp Chrysalidis in 2025
# ======================================================================

class SignalToTargetAssignmentSet(SuperAssignmentSetType):
  def __init__(self):
    #attributes
    self.id = None
    self.name = None
    ...
    self.signalToTargetAssignment_list = list()  #Initialize as an empty list for multiple children
    self.parent = None #default is no parent, but can be set for every element
	
    #Check if it's a collection (i.e., upperBound <> 1)
  def add_signalToTargetAssignment(self, signalToTargetAssignment): #function for adding child
    self.signalToTargetAssignment_list.append(signalToTargetAssignment)
    signalToTargetAssignment.parent = self #link child to parent

  def to_xml(self, reference_name = None):
    element = ET.Element(reference_name)
    if not reference_name == "database" and not reference_name.lower() == "signaltotargetassignmentset":
      element.set("xsi:type", "de.ucof.model.assignments.database:SignalToTargetAssignmentSet")  
  ...
  # Multiple children
    for child in self.signalToTargetAssignment_list:  
      element.append(child.to_xml("signalToTargetAssignment"))  
    return element

  def from_xml(self, xml_element):
    # Instantiate an object of type SignalToTargetAssignmentSet
    obj = SignalToTargetAssignmentSet()  
    ...
    obj.signalToTargetAssignment_list = list()
    for child_element in xml_element.findall("signalToTargetAssignment"):
      xsi_type = child_element.get("{http://www.w3.org/2001/XMLSchema-instance}type")
      if not xsi_type:
        child = SignalToTargetAssignment.from_xml(self, child_element)
        child.parent = obj
        obj.signalToTargetAssignment_list.append(child)
