...
# -------------------------------------------------------------------
# Automatically generated class from ecore model!! TriggerWords.
# Don't change here, change in source ecore model
# Code generator by Philipp Chrysalidis in 2025
# ======================================================================

class TriggerWords(SuperIdentifierType, SuperTrackingType):
  def __init__(self):
    #attributes
    self.id = None
    self.name = None
    ...
    self.TriggerWord_list = list()  #Initialize as an empty list for multiple children
    self.parent = None #default is no parent, but can be set for every element
	
    #Check if it's a collection (i.e., upperBound <> 1)
  def add_TriggerWord(self, TriggerWord): #function for adding child
    self.TriggerWord_list.append(TriggerWord)
    TriggerWord.parent = self #link child to parent

  def to_xml(self, reference_name = None):
    element = ET.Element(reference_name)
    if not reference_name == "packageStrategyTypes" and not reference_name.lower() == "triggerwords":
      element.set("xsi:type", "de.ucof.model.communication.configECIC.packageStrategyTypes:TriggerWords")  
  ...
  # Multiple children
    for child in self.TriggerWord_list:  
      element.append(child.to_xml("TriggerWord"))  
    return element

  def from_xml(self, xml_element):
    # Instantiate an object of type TriggerWords
    obj = TriggerWords()  
    ...
    obj.TriggerWord_list = list()
    for child_element in xml_element.findall("TriggerWord"):
      xsi_type = child_element.get("{http://www.w3.org/2001/XMLSchema-instance}type")
      if not xsi_type:
        child = TriggerWord.from_xml(self, child_element)
        child.parent = obj
        obj.TriggerWord_list.append(child)
