...
from voragoPorts import voragoPorts

from MsgBoxExtension import MsgBoxExtension

from CanDriverConfig import CanDriverConfig


# -------------------------------------------------------------------
# Automatically generated class from ecore model!! va41620peb1.
# Don't change here, change in source ecore model
# Code generator by Philipp Chrysalidis in 2025
# ======================================================================

class va41620peb1(SuperTargetType, SuperPowerType):
  def __init__(self):
    #attributes
    self.id = None
    self.name = None
    ...
    self.canDriverConfig = None  #Initialize as a single child (None by default)
  ...
  def to_xml(self, reference_name = None):
    ...
    if not reference_name == "cpn" and not reference_name.lower() == "va41620peb1":
      element.set("xsi:type", "de.ucof.model.targets.cpn:va41620peb1")  
  ...
  # Single child
    if self.canDriverConfig is not None:
      element.append(self.canDriverConfig.to_xml("canDriverConfig"))  
    return element

  def from_xml(self, xml_element):
    # Instantiate an object of type va41620peb1
    obj = va41620peb1()  
    ...
    if child_element is not None:  
      xsi_type = child_element.get("{http://www.w3.org/2001/XMLSchema-instance}type")
      if (not xsi_type) or (xsi_type.split(":", 1)[ 1 ].lower() == "voragoports"):
        child = voragoPorts.from_xml(self, child_element)
        child.parent = obj
        obj.ports = child        
    ...
    # Reference with a single value: find the child element and set the field if it exists
    child_element = xml_element.find("canDriverConfig")
    if child_element is not None:  
      xsi_type = child_element.get("{http://www.w3.org/2001/XMLSchema-instance}type")
      if (not xsi_type) or (xsi_type.split(":", 1)[ 1 ].lower() == "candriverconfig"):
        child = CanDriverConfig.from_xml(self, child_element)
        child.parent = obj
        obj.canDriverConfig = child        
