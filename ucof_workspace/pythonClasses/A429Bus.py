...
# -------------------------------------------------------------------
# Automatically generated class from ecore model!! A429Bus.
# Don't change here, change in source ecore model
# Code generator by Philipp Chrysalidis in 2025
# ======================================================================

class A429Bus(SuperCommunicationInterfaceType):
  def __init__(self):
    #attributes
    self.id = None
    self.name = None
  ...
  def to_xml(self, reference_name = None):
    ...
    if not reference_name == "arinc429" and not reference_name.lower() == "a429bus":
      element.set("xsi:type", "de.ucof.model.communication.arinc429:A429Bus")  
    ...
    if self.parityType is not None:
      element.set("parityType", str(self.parityType))  
    # Convert children to nested XML elements
    return element

  def from_xml(self, xml_element):
    # Instantiate an object of type A429Bus
    obj = A429Bus()  
    ...
    if "parityType" in xml_element.attrib:
        obj.parityType = xml_element.get("parityType")
