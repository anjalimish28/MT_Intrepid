...
from A664PacketStrategy import A664PacketStrategy


# -------------------------------------------------------------------
# Automatically generated class from ecore model!! A664Stream.
# Don't change here, change in source ecore model
# Code generator by Philipp Chrysalidis in 2025
# ======================================================================

class A664Stream(SuperStreamType):
  def __init__(self):
    #attributes
    self.id = None
    self.name = None
    ...
    self.icd = None
    self.direction = None
    self.sampleMaxSizeBytes = None
    self.errors = None
    #children (references)
    self.dataTimestamp = None  #Initialize as a single child (None by default)
    self.a664PacketStrategy = None  #Initialize as a single child (None by default)
  ...
  def to_xml(self, reference_name = None):
    ...
    if not reference_name == "streamTypes" and not reference_name.lower() == "a664stream":
      element.set("xsi:type", "de.ucof.model.communication.configECIC.streamTypes:A664Stream")  
    ...
    if self.sampleMaxSizeBytes is not None:
      element.set("sampleMaxSizeBytes", str(self.sampleMaxSizeBytes))  
  ...
  # Single child
    if self.a664PacketStrategy is not None:
      element.append(self.a664PacketStrategy.to_xml("a664PacketStrategy"))  
    return element

  def from_xml(self, xml_element):
    # Instantiate an object of type A664Stream
    obj = A664Stream()  
    ...
    if "sampleMaxSizeBytes" in xml_element.attrib:
        obj.sampleMaxSizeBytes = xml_element.get("sampleMaxSizeBytes")
    ...
    # Reference with a single value: find the child element and set the field if it exists
    child_element = xml_element.find("a664PacketStrategy")
    if child_element is not None:  
      xsi_type = child_element.get("{http://www.w3.org/2001/XMLSchema-instance}type")
      if (not xsi_type) or (xsi_type.split(":", 1)[ 1 ].lower() == "a664packetstrategy"):
        child = A664PacketStrategy.from_xml(self, child_element)
        child.parent = obj
        obj.a664PacketStrategy = child        
