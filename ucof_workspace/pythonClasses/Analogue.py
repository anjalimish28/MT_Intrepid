...
class Analogue(WiredSuper):
  def __init__(self):
    ...
    self.creator = None
    self.version = None
    self.incCountStepSize = None
    self.decCountStepSize = None
    #children (references)
    self.connector = None  #Initialize as a single child (None by default)
  ...
  def to_xml(self, reference_name = None):
    ...
    if not reference_name == "basedonwired" and not reference_name.lower() == "analogue":
      element.set("xsi:type", "de.ucof.model.communication.basedonwired:Analogue")  
    ...
    if self.incCountStepSize is not None:
      element.set("incCountStepSize", str(self.incCountStepSize))  
    if self.decCountStepSize is not None:
      element.set("decCountStepSize", str(self.decCountStepSize))  
    # Convert children to nested XML elements
  # Single child
    if self.connector is not None:
      element.append(self.connector.to_xml("connector"))  
    return element

  def from_xml(self, xml_element):
    # Instantiate an object of type Analogue
    obj = Analogue()  
    ...
    if "incCountStepSize" in xml_element.attrib:
        obj.incCountStepSize = xml_element.get("incCountStepSize")
    if "decCountStepSize" in xml_element.attrib:
        obj.decCountStepSize = xml_element.get("decCountStepSize")
    # Process children (references)
    # Reference with a single value: find the child element and set the field if it exists
    child_element = xml_element.find("connector")
    if child_element is not None:  
      xsi_type = child_element.get("{http://www.w3.org/2001/XMLSchema-instance}type")
      if (not xsi_type) or (xsi_type.split(":", 1)[ 1 ].lower() == "connector"):
        child = Connector.from_xml(self, child_element)
        child.parent = obj
        obj.connector = child        
