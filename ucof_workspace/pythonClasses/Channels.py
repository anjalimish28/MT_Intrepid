...
# -------------------------------------------------------------------
# Automatically generated class from ecore model!! Channels.
# Don't change here, change in source ecore model
# Code generator by Philipp Chrysalidis in 2025
# ======================================================================

class Channels(SuperIdentifierType, SuperTrackingType):
  def __init__(self):
    #attributes
    self.id = None
    self.name = None
    ...
    self.channel_list = list()  #Initialize as an empty list for multiple children
    self.parent = None #default is no parent, but can be set for every element
	
    #Check if it's a collection (i.e., upperBound <> 1)
  def add_channel(self, channel): #function for adding child
    self.channel_list.append(channel)
    channel.parent = self #link child to parent

  def to_xml(self, reference_name = None):
    element = ET.Element(reference_name)
    if not reference_name == "configECIC" and not reference_name.lower() == "channels":
      element.set("xsi:type", "de.ucof.model.communication.configECIC:Channels")  
  ...
  # Multiple children
    for child in self.channel_list:  
      element.append(child.to_xml("channel"))  
    return element

  def from_xml(self, xml_element):
    # Instantiate an object of type Channels
    obj = Channels()  
    ...
    obj.channel_list = list()
    for child_element in xml_element.findall("channel"):
      xsi_type = child_element.get("{http://www.w3.org/2001/XMLSchema-instance}type")
      if not xsi_type:
        child = Channel.from_xml(self, child_element)
        child.parent = obj
        obj.channel_list.append(child)
