...
from UDPSockets import UDPSockets


# -------------------------------------------------------------------
# Automatically generated class from ecore model!! ComInterface.
# Don't change here, change in source ecore model
# Code generator by Philipp Chrysalidis in 2025
# ======================================================================

class ComInterface(SuperIdentifierType, SuperTrackingType):
  def __init__(self):
    #attributes
    self.id = None
    self.name = None
    ...
    self.version = None
    #children (references)
    self.udpSockets = None  #Initialize as a single child (None by default)
  ...
  def to_xml(self, reference_name = None):
    ...
    if not reference_name == "configECIC" and not reference_name.lower() == "cominterface":
      element.set("xsi:type", "de.ucof.model.communication.configECIC:ComInterface")  
  ...
  # Single child
    if self.udpSockets is not None:
      element.append(self.udpSockets.to_xml("udpSockets"))  
    return element

  def from_xml(self, xml_element):
    # Instantiate an object of type ComInterface
    obj = ComInterface()  
    ...
    child_element = xml_element.find("udpSockets")
    if child_element is not None:  
      xsi_type = child_element.get("{http://www.w3.org/2001/XMLSchema-instance}type")
      if (not xsi_type) or (xsi_type.split(":", 1)[ 1 ].lower() == "udpsockets"):
        child = UDPSockets.from_xml(self, child_element)
        child.parent = obj
        obj.udpSockets = child        
