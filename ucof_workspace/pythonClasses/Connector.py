...
from ConnectionPin import ConnectionPin


# -------------------------------------------------------------------
# Automatically generated class from ecore model!! Connector.
# Don't change here, change in source ecore model
# Code generator by Philipp Chrysalidis in 2025
# ======================================================================

class Connector(SuperIdentifierType, SuperTrackingType):
  def __init__(self):
    #attributes
    self.id = None
    self.name = None
    ...
    self.connectionPin_list = list()  #Initialize as an empty list for multiple children
    self.parent = None #default is no parent, but can be set for every element
	
    #Check if it's a collection (i.e., upperBound <> 1)
  def add_connectionPin(self, connectionPin): #function for adding child
    self.connectionPin_list.append(connectionPin)
    connectionPin.parent = self #link child to parent

  def to_xml(self, reference_name = None):
    element = ET.Element(reference_name)
    if not reference_name == "connectors" and not reference_name.lower() == "connector":
      element.set("xsi:type", "de.ucof.model.communication.connectors:Connector")  
    ...
    if self.connectorType is not None:
      element.set("connectorType", str(self.connectorType))  
    if self.lineType is not None:
      element.set("lineType", str(self.lineType))  
    # Convert children to nested XML elements
  # Multiple children
    for child in self.connectionPin_list:  
      element.append(child.to_xml("connectionPin"))  
    return element

  def from_xml(self, xml_element):
    # Instantiate an object of type Connector
    obj = Connector()  
    ...
    if "connectorType" in xml_element.attrib:
        obj.connectorType = xml_element.get("connectorType")
    if "lineType" in xml_element.attrib:
        obj.lineType = xml_element.get("lineType")
    # Process children (references)
    # Reference with multiplicity: initialize a list and append each child object
    obj.connectionPin_list = list()
    for child_element in xml_element.findall("connectionPin"):
      xsi_type = child_element.get("{http://www.w3.org/2001/XMLSchema-instance}type")
      if not xsi_type:
        child = ConnectionPin.from_xml(self, child_element)
        child.parent = obj
        obj.connectionPin_list.append(child)
