...
from DISPacketStrategy import DISPacketStrategy


# -------------------------------------------------------------------
# Automatically generated class from ecore model!! DISStream.
# Don't change here, change in source ecore model
# Code generator by Philipp Chrysalidis in 2025
# ======================================================================

class DISStream(SuperStreamType):
  def __init__(self):
    #attributes
    self.id = None
    self.name = None
    ...
    self.disPacketStrategy = None  #Initialize as a single child (None by default)
  ...
  def to_xml(self, reference_name = None):
    ...
    if not reference_name == "streamTypes" and not reference_name.lower() == "disstream":
      element.set("xsi:type", "de.ucof.model.communication.configECIC.streamTypes:DISStream")  
  ...
  # Single child
    if self.disPacketStrategy is not None:
      element.append(self.disPacketStrategy.to_xml("disPacketStrategy"))  
    return element

  def from_xml(self, xml_element):
    # Instantiate an object of type DISStream
    obj = DISStream()  
    ...
    # Reference with a single value: find the child element and set the field if it exists
    child_element = xml_element.find("disPacketStrategy")
    if child_element is not None:  
      xsi_type = child_element.get("{http://www.w3.org/2001/XMLSchema-instance}type")
      if (not xsi_type) or (xsi_type.split(":", 1)[ 1 ].lower() == "dispacketstrategy"):
        child = DISPacketStrategy.from_xml(self, child_element)
        child.parent = obj
        obj.disPacketStrategy = child        
