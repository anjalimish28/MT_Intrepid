...
from InterfaceSet import InterfaceSet

from MessageSet import MessageSet

from PayloadSet import PayloadSet

from SignalSet import SignalSet

# -------------------------------------------------------------------
# Automatically generated class from ecore model!! Database.
# Don't change here, change in source ecore model
# Code generator by Philipp Chrysalidis in 2025
# ======================================================================

class Database(SuperIdentifierType, SuperTrackingType):
  def __init__(self):
    #attributes
    self.id = None
    self.name = None
    ...
    self.communicationSet_list = list()  #Initialize as an empty list for multiple children
    self.parent = None #default is no parent, but can be set for every element
	
    #Check if it's a collection (i.e., upperBound <> 1)
  def add_communicationSet(self, communicationSet): #function for adding child
    self.communicationSet_list.append(communicationSet)
    communicationSet.parent = self #link child to parent

  def to_xml(self, reference_name = None):
    element = ET.Element(reference_name)
    if not reference_name == "communication" and not reference_name.lower() == "database":
      element.set("xsi:type", "de.ucof.model.communication:Database")  
  ...
  # Multiple children
    for child in self.communicationSet_list:  
      element.append(child.to_xml("communicationSet"))  
    return element

  def from_xml(self, xml_element):
    # Instantiate an object of type Database
    obj = Database()  
    ...
    obj.communicationSet_list = list()
    for child_element in xml_element.findall("communicationSet"):
      xsi_type = child_element.get("{http://www.w3.org/2001/XMLSchema-instance}type")
      if not xsi_type:
        child = CommunicationSetType.from_xml(self, child_element)
        child.parent = obj
        obj.communicationSet_list.append(child)
      elif xsi_type.split(":", 1)[ 1 ].lower() == "interfaceset":
        child = InterfaceSet.from_xml(self, child_element)
        child.parent = obj
        obj.communicationSet_list.append(child)
      
      elif xsi_type.split(":", 1)[ 1 ].lower() == "messageset":
        child = MessageSet.from_xml(self, child_element)
        child.parent = obj
        obj.communicationSet_list.append(child)
      
      elif xsi_type.split(":", 1)[ 1 ].lower() == "payloadset":
        child = PayloadSet.from_xml(self, child_element)
        child.parent = obj
        obj.communicationSet_list.append(child)
      
      elif xsi_type.split(":", 1)[ 1 ].lower() == "signalset":
        child = SignalSet.from_xml(self, child_element)
        child.parent = obj
        obj.communicationSet_list.append(child)
    return obj
