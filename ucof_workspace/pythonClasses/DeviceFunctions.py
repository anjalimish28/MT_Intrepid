...
# -------------------------------------------------------------------
# Automatically generated class from ecore model!! DeviceFunctions.
# Don't change here, change in source ecore model
# Code generator by Philipp Chrysalidis in 2025
# ======================================================================

class DeviceFunctions(SuperIdentifierType, SuperTrackingType):
  def __init__(self):
    #attributes
    self.id = None
    self.name = None
    ...
    self.function_list = list()  #Initialize as an empty list for multiple children
    self.parent = None #default is no parent, but can be set for every element
	
    #Check if it's a collection (i.e., upperBound <> 1)
  def add_function(self, function): #function for adding child
    self.function_list.append(function)
    function.parent = self #link child to parent

  def to_xml(self, reference_name = None):
    element = ET.Element(reference_name)
    if not reference_name == "functions" and not reference_name.lower() == "devicefunctions":
      element.set("xsi:type", "de.ucof.model.devices.functions:DeviceFunctions")  
  ...
  # Multiple children
    for child in self.function_list:  
      element.append(child.to_xml("function"))  
    return element

  def from_xml(self, xml_element):
    # Instantiate an object of type DeviceFunctions
    obj = DeviceFunctions()  
    ...
    obj.function_list = list()
    for child_element in xml_element.findall("function"):
      xsi_type = child_element.get("{http://www.w3.org/2001/XMLSchema-instance}type")
      if not xsi_type:
        child = SuperFunctionType.from_xml(self, child_element)
        child.parent = obj
        obj.function_list.append(child)
      elif xsi_type.split(":", 1)[ 1 ].lower() == "routing":
        child = Routing.from_xml(self, child_element)
        child.parent = obj
        obj.function_list.append(child)
    return obj
