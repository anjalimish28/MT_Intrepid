...
from ED247Assignment import ED247Assignment


# -------------------------------------------------------------------
# Automatically generated class from ecore model!! ED247AssignmentSet.
# Don't change here, change in source ecore model
# Code generator by Philipp Chrysalidis in 2025
# ======================================================================

class ED247AssignmentSet(SuperAssignmentSetType):
  def __init__(self):
    #attributes
    self.id = None
    self.name = None
    ...
    self.ed247Assignment_list = list()  #Initialize as an empty list for multiple children
    self.parent = None #default is no parent, but can be set for every element
	
    #Check if it's a collection (i.e., upperBound <> 1)
  def add_ed247Assignment(self, ed247Assignment): #function for adding child
    self.ed247Assignment_list.append(ed247Assignment)
    ed247Assignment.parent = self #link child to parent

  def to_xml(self, reference_name = None):
    element = ET.Element(reference_name)
    if not reference_name == "assignments" and not reference_name.lower() == "ed247assignmentset":
      element.set("xsi:type", "de.ucof.model.assignments:ED247AssignmentSet")  
  ...
  # Multiple children
    for child in self.ed247Assignment_list:  
      element.append(child.to_xml("ed247Assignment"))  
    return element

  def from_xml(self, xml_element):
    # Instantiate an object of type ED247AssignmentSet
    obj = ED247AssignmentSet()  
    ...
    obj.ed247Assignment_list = list()
    for child_element in xml_element.findall("ed247Assignment"):
      xsi_type = child_element.get("{http://www.w3.org/2001/XMLSchema-instance}type")
      if not xsi_type:
        child = ED247Assignment.from_xml(self, child_element)
        child.parent = obj
        obj.ed247Assignment_list.append(child)
