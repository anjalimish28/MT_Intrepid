...
from ED247Configuration import ED247Configuration


# -------------------------------------------------------------------
# Automatically generated class from ecore model!! ED247Configurations.
# Don't change here, change in source ecore model
# Code generator by Philipp Chrysalidis in 2025
# ======================================================================

class ED247Configurations(SuperIdentifierType, SuperTrackingType):
  def __init__(self):
    #attributes
    self.id = None
    self.name = None
    ...
    self.ed247Configuration_list = list()  #Initialize as an empty list for multiple children
    self.parent = None #default is no parent, but can be set for every element
	
    #Check if it's a collection (i.e., upperBound <> 1)
  def add_ed247Configuration(self, ed247Configuration): #function for adding child
    self.ed247Configuration_list.append(ed247Configuration)
    ed247Configuration.parent = self #link child to parent

  def to_xml(self, reference_name = None):
    element = ET.Element(reference_name)
    if not reference_name == "configECIC" and not reference_name.lower() == "ed247configurations":
      element.set("xsi:type", "de.ucof.model.communication.configECIC:ED247Configurations")  
  ...
  # Multiple children
    for child in self.ed247Configuration_list:  
      element.append(child.to_xml("ed247Configuration"))  
    return element

  def from_xml(self, xml_element):
    # Instantiate an object of type ED247Configurations
    obj = ED247Configurations()  
    ...
    obj.ed247Configuration_list = list()
    for child_element in xml_element.findall("ed247Configuration"):
      xsi_type = child_element.get("{http://www.w3.org/2001/XMLSchema-instance}type")
      if not xsi_type:
        child = ED247Configuration.from_xml(self, child_element)
        child.parent = obj
        obj.ed247Configuration_list.append(child)
