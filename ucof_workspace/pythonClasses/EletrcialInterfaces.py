...
from SuperElectricalInterfaceType import SuperElectricalInterfaceType
from adcPort import adcPort

from sheildPort import sheildPort

from intrepidElectricalInterface import intrepidElectricalInterface

# -------------------------------------------------------------------
# Automatically generated class from ecore model!! EletrcialInterfaces.
# Don't change here, change in source ecore model
# Code generator by Philipp Chrysalidis in 2025
# ======================================================================

class EletrcialInterfaces(SuperIdentifierType, SuperTrackingType):
  def __init__(self):
    #attributes
    self.id = None
    self.name = None
    ...
    self.electricalInterface_list = list()  #Initialize as an empty list for multiple children
    self.parent = None #default is no parent, but can be set for every element
	
    #Check if it's a collection (i.e., upperBound <> 1)
  def add_electricalInterface(self, electricalInterface): #function for adding child
    self.electricalInterface_list.append(electricalInterface)
    electricalInterface.parent = self #link child to parent

  def to_xml(self, reference_name = None):
    element = ET.Element(reference_name)
    if not reference_name == "powergrid" and not reference_name.lower() == "eletrcialinterfaces":
      element.set("xsi:type", "de.ucof.model.targets.powergrid:EletrcialInterfaces")  
  ...
  # Multiple children
    for child in self.electricalInterface_list:  
      element.append(child.to_xml("electricalInterface"))  
    return element

  def from_xml(self, xml_element):
    # Instantiate an object of type EletrcialInterfaces
    obj = EletrcialInterfaces()  
    ...
    obj.electricalInterface_list = list()
    for child_element in xml_element.findall("electricalInterface"):
      xsi_type = child_element.get("{http://www.w3.org/2001/XMLSchema-instance}type")
      if not xsi_type:
        child = SuperElectricalInterfaceType.from_xml(self, child_element)
        child.parent = obj
        obj.electricalInterface_list.append(child)
      elif xsi_type.split(":", 1)[ 1 ].lower() == "adcport":
        child = adcPort.from_xml(self, child_element)
        child.parent = obj
        obj.electricalInterface_list.append(child)
      
      elif xsi_type.split(":", 1)[ 1 ].lower() == "sheildport":
        child = sheildPort.from_xml(self, child_element)
        child.parent = obj
        obj.electricalInterface_list.append(child)
      
      elif xsi_type.split(":", 1)[ 1 ].lower() == "intrepidelectricalinterface":
        child = intrepidElectricalInterface.from_xml(self, child_element)
        child.parent = obj
        obj.electricalInterface_list.append(child)
    return obj
