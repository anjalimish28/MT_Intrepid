...
# -------------------------------------------------------------------
# Automatically generated class from ecore model!! Frame.
# Don't change here, change in source ecore model
# Code generator by Philipp Chrysalidis in 2025
# ======================================================================

class Frame(SuperIdentifierType, SuperTrackingType):
  def __init__(self):
    #attributes
    self.id = None
    self.name = None
  ...
  def to_xml(self, reference_name = None):
    ...
    if not reference_name == "packageStrategyTypes" and not reference_name.lower() == "frame":
      element.set("xsi:type", "de.ucof.model.communication.configECIC.packageStrategyTypes:Frame")  
    ...
    if self.A825ID is not None:
      element.set("A825ID", str(self.A825ID))  
    if self.ResetPeriodicTimer is not None:
      element.set("ResetPeriodicTimer", str(self.ResetPeriodicTimer))  
    # Convert children to nested XML elements
    return element

  def from_xml(self, xml_element):
    # Instantiate an object of type Frame
    obj = Frame()  
    ...
    if "A825ID" in xml_element.attrib:
        obj.A825ID = xml_element.get("A825ID")
    if "ResetPeriodicTimer" in xml_element.attrib:
        obj.ResetPeriodicTimer = xml_element.get("ResetPeriodicTimer")
