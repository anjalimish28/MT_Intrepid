...
# -------------------------------------------------------------------
# Automatically generated class from ecore model!! Frames.
# Don't change here, change in source ecore model
# Code generator by Philipp Chrysalidis in 2025
# ======================================================================

class Frames(SuperIdentifierType, SuperTrackingType):
  def __init__(self):
    #attributes
    self.id = None
    self.name = None
    ...
    self.version = None
    #children (references)
    self.Frame = None  #Initialize as a single child (None by default)
    self.LLC = None  #Initialize as a single child (None by default)
  ...
  def to_xml(self, reference_name = None):
    ...
    if not reference_name == "packageStrategyTypes" and not reference_name.lower() == "frames":
      element.set("xsi:type", "de.ucof.model.communication.configECIC.packageStrategyTypes:Frames")  
  ...
  # Single child
    if self.Frame is not None:
      element.append(self.Frame.to_xml("Frame"))  
  # Single child
    if self.LLC is not None:
      element.append(self.LLC.to_xml("LLC"))  
    return element

  def from_xml(self, xml_element):
    # Instantiate an object of type Frames
    obj = Frames()  
    ...
    child_element = xml_element.find("Frame")
    if child_element is not None:  
      xsi_type = child_element.get("{http://www.w3.org/2001/XMLSchema-instance}type")
      if (not xsi_type) or (xsi_type.split(":", 1)[ 1 ].lower() == "frame"):
        child = Frame.from_xml(self, child_element)
        child.parent = obj
        obj.Frame = child        
      
    # Reference with a single value: find the child element and set the field if it exists
    child_element = xml_element.find("LLC")
    if child_element is not None:  
      xsi_type = child_element.get("{http://www.w3.org/2001/XMLSchema-instance}type")
      if (not xsi_type) or (xsi_type.split(":", 1)[ 1 ].lower() == "llc"):
        child = LLC.from_xml(self, child_element)
        child.parent = obj
        obj.LLC = child        
