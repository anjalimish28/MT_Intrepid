...
from SuperCommunicationInterfaceType import SuperCommunicationInterfaceType
from A429Bus import A429Bus

from A825Bus import A825Bus

from EthernetPort import EthernetPort
from A664Bus import A664Bus
from WiredInterfaces import WiredInterfaces

from I2CBus import I2CBus

# -------------------------------------------------------------------
# Automatically generated class from ecore model!! InterfaceSet.
# Don't change here, change in source ecore model
# Code generator by Philipp Chrysalidis in 2025
# ======================================================================

class InterfaceSet(CommunicationSetType):
  def __init__(self):
    #attributes
    self.id = None
    self.name = None
    ...
    self.interface_list = list()  #Initialize as an empty list for multiple children
    self.parent = None #default is no parent, but can be set for every element
	
    #Check if it's a collection (i.e., upperBound <> 1)
  def add_interface(self, interface): #function for adding child
    self.interface_list.append(interface)
    interface.parent = self #link child to parent

  def to_xml(self, reference_name = None):
    element = ET.Element(reference_name)
    if not reference_name == "communication" and not reference_name.lower() == "interfaceset":
      element.set("xsi:type", "de.ucof.model.communication:InterfaceSet")  
  ...
  # Multiple children
    for child in self.interface_list:  
      element.append(child.to_xml("interface"))  
    return element

  def from_xml(self, xml_element):
    # Instantiate an object of type InterfaceSet
    obj = InterfaceSet()  
    ...
    obj.interface_list = list()
    for child_element in xml_element.findall("interface"):
      xsi_type = child_element.get("{http://www.w3.org/2001/XMLSchema-instance}type")
      if not xsi_type:
        child = SuperCommunicationInterfaceType.from_xml(self, child_element)
        child.parent = obj
        obj.interface_list.append(child)
      elif xsi_type.split(":", 1)[ 1 ].lower() == "a429bus":
        child = A429Bus.from_xml(self, child_element)
        child.parent = obj
        obj.interface_list.append(child)
      
      elif xsi_type.split(":", 1)[ 1 ].lower() == "a825bus":
        child = A825Bus.from_xml(self, child_element)
        child.parent = obj
        obj.interface_list.append(child)
      
      elif xsi_type.split(":", 1)[ 1 ].lower() == "ethernetport":
        child = EthernetPort.from_xml(self, child_element)
        child.parent = obj
        obj.interface_list.append(child)
      elif xsi_type.split(":", 1)[ 1 ].lower() == "a664bus":
        child = A664Bus.from_xml(self, child_element)
        child.parent = obj
        obj.interface_list.append(child)
      elif xsi_type.split(":", 1)[ 1 ].lower() == "wiredinterfaces":
        child = WiredInterfaces.from_xml(self, child_element)
        child.parent = obj
        obj.interface_list.append(child)
      
      elif xsi_type.split(":", 1)[ 1 ].lower() == "i2cbus":
        child = I2CBus.from_xml(self, child_element)
        child.parent = obj
        obj.interface_list.append(child)
    return obj
