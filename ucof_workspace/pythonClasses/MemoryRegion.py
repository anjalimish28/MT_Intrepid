...
# -------------------------------------------------------------------
# Automatically generated class from ecore model!! MemoryRegion.
# Don't change here, change in source ecore model
# Code generator by Philipp Chrysalidis in 2025
# ======================================================================

class MemoryRegion(SuperIdentifierType, SuperTrackingType):
  def __init__(self):
    #attributes
    self.id = None
    self.name = None
  ...
  def to_xml(self, reference_name = None):
    ...
    if not reference_name == "a653components" and not reference_name.lower() == "memoryregion":
      element.set("xsi:type", "de.ucof.model.a653components:MemoryRegion")  
    ...
    if self.accessRights is not None:
      element.set("accessRights", str(self.accessRights))  
    # Convert children to nested XML elements
    return element

  def from_xml(self, xml_element):
    # Instantiate an object of type MemoryRegion
    obj = MemoryRegion()  
    ...
    if "accessRights" in xml_element.attrib:
        obj.accessRights = xml_element.get("accessRights")
