...
# -------------------------------------------------------------------
# Automatically generated class from ecore model!! ModulePartitions.
# Don't change here, change in source ecore model
# Code generator by Philipp Chrysalidis in 2025
# ======================================================================

class ModulePartitions(SuperIdentifierType):
  def __init__(self):
    #attributes
    self.id = None
    self.name = None
    ...
    self.partition_list = list()  #Initialize as an empty list for multiple children
    self.parent = None #default is no parent, but can be set for every element
	
    #Check if it's a collection (i.e., upperBound <> 1)
  def add_partition(self, partition): #function for adding child
    self.partition_list.append(partition)
    partition.parent = self #link child to parent

  def to_xml(self, reference_name = None):
    element = ET.Element(reference_name)
    if not reference_name == "a653partition" and not reference_name.lower() == "modulepartitions":
      element.set("xsi:type", "de.ucof.model.a653partition:ModulePartitions")  
  ...
  # Multiple children
    for child in self.partition_list:  
      element.append(child.to_xml("partition"))  
    return element

  def from_xml(self, xml_element):
    # Instantiate an object of type ModulePartitions
    obj = ModulePartitions()  
    ...
    obj.partition_list = list()
    for child_element in xml_element.findall("partition"):
      xsi_type = child_element.get("{http://www.w3.org/2001/XMLSchema-instance}type")
      if not xsi_type:
        child = Partition.from_xml(self, child_element)
        child.parent = obj
        obj.partition_list.append(child)
