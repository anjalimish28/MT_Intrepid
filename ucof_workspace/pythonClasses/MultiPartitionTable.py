...
from ErrorInfoMultiPartition import ErrorInfoMultiPartition


# -------------------------------------------------------------------
# Automatically generated class from ecore model!! MultiPartitionTable.
# Don't change here, change in source ecore model
# Code generator by Philipp Chrysalidis in 2025
# ======================================================================

class MultiPartitionTable(SuperIdentifierType, SuperTrackingType):
  def __init__(self):
    #attributes
    self.id = None
    self.name = None
  ...
  def to_xml(self, reference_name = None):
    ...
    if not reference_name == "healthmanagement" and not reference_name.lower() == "multipartitiontable":
      element.set("xsi:type", "de.ucof.model.healthmanagement:MultiPartitionTable")  
    ...
    if self.tableID is not None:
      element.set("tableID", str(self.tableID))  
  ...
  def from_xml(self, xml_element):
    # Instantiate an object of type MultiPartitionTable
    obj = MultiPartitionTable()  
    ...
    if "tableID" in xml_element.attrib:
        obj.tableID = xml_element.get("tableID")
    ...
    for child_element in xml_element.findall("errorInfo"):
      ...
      if not xsi_type:
        child = ErrorInfoMultiPartition.from_xml(self, child_element)
        child.parent = obj
