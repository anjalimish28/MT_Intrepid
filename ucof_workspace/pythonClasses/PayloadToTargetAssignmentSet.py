...
from PayloadToTargetAssignment import PayloadToTargetAssignment


# -------------------------------------------------------------------
# Automatically generated class from ecore model!! PayloadToTargetAssignmentSet.
# Don't change here, change in source ecore model
# Code generator by Philipp Chrysalidis in 2025
# ======================================================================

class PayloadToTargetAssignmentSet(SuperAssignmentSetType):
  def __init__(self):
    #attributes
    self.id = None
    self.name = None
    ...
    self.payloadToTargetAssignment_list = list()  #Initialize as an empty list for multiple children
    self.parent = None #default is no parent, but can be set for every element
	
    #Check if it's a collection (i.e., upperBound <> 1)
  def add_payloadToTargetAssignment(self, payloadToTargetAssignment): #function for adding child
    self.payloadToTargetAssignment_list.append(payloadToTargetAssignment)
    payloadToTargetAssignment.parent = self #link child to parent

  def to_xml(self, reference_name = None):
    element = ET.Element(reference_name)
    if not reference_name == "database" and not reference_name.lower() == "payloadtotargetassignmentset":
      element.set("xsi:type", "de.ucof.model.assignments.database:PayloadToTargetAssignmentSet")  
  ...
  # Multiple children
    for child in self.payloadToTargetAssignment_list:  
      element.append(child.to_xml("payloadToTargetAssignment"))  
    return element

  def from_xml(self, xml_element):
    # Instantiate an object of type PayloadToTargetAssignmentSet
    obj = PayloadToTargetAssignmentSet()  
    ...
    obj.payloadToTargetAssignment_list = list()
    for child_element in xml_element.findall("payloadToTargetAssignment"):
      xsi_type = child_element.get("{http://www.w3.org/2001/XMLSchema-instance}type")
      if not xsi_type:
        child = PayloadToTargetAssignment.from_xml(self, child_element)
        child.parent = obj
        obj.payloadToTargetAssignment_list.append(child)
