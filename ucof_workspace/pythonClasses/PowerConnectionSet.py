...
from PowerConnection import PowerConnection


# -------------------------------------------------------------------
# Automatically generated class from ecore model!! PowerConnectionSet.
# Don't change here, change in source ecore model
# Code generator by Philipp Chrysalidis in 2025
# ======================================================================

class PowerConnectionSet(SuperIdentifierType, SuperTrackingType):
  def __init__(self):
    #attributes
    self.id = None
    self.name = None
    ...
    self.powerConnection_list = list()  #Initialize as an empty list for multiple children
    self.parent = None #default is no parent, but can be set for every element
	
    #Check if it's a collection (i.e., upperBound <> 1)
  def add_powerConnection(self, powerConnection): #function for adding child
    self.powerConnection_list.append(powerConnection)
    powerConnection.parent = self #link child to parent

  def to_xml(self, reference_name = None):
    element = ET.Element(reference_name)
    if not reference_name == "powergrid" and not reference_name.lower() == "powerconnectionset":
      element.set("xsi:type", "de.ucof.model.targets.powergrid:PowerConnectionSet")  
  ...
  # Multiple children
    for child in self.powerConnection_list:  
      element.append(child.to_xml("powerConnection"))  
    return element

  def from_xml(self, xml_element):
    # Instantiate an object of type PowerConnectionSet
    obj = PowerConnectionSet()  
    ...
    obj.powerConnection_list = list()
    for child_element in xml_element.findall("powerConnection"):
      xsi_type = child_element.get("{http://www.w3.org/2001/XMLSchema-instance}type")
      if not xsi_type:
        child = PowerConnection.from_xml(self, child_element)
        child.parent = obj
        obj.powerConnection_list.append(child)
