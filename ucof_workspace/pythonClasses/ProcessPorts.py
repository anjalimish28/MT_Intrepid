...
# -------------------------------------------------------------------
# Automatically generated class from ecore model!! ProcessPorts.
# Don't change here, change in source ecore model
# Code generator by Philipp Chrysalidis in 2025
# ======================================================================

class ProcessPorts(SuperTrackingType, SuperIdentifierType):
  def __init__(self):
    #attributes
  ...
  def to_xml(self, reference_name = None):
    ...
    if not reference_name == "ports" and not reference_name.lower() == "processports":
      element.set("xsi:type", "de.ucof.model.devices.ports:ProcessPorts")  
  ...
  def from_xml(self, xml_element):
    # Instantiate an object of type ProcessPorts
    obj = ProcessPorts()  
    ...
    for child_element in xml_element.findall("port"):
      ...
      if not xsi_type:
        child = SuperIntraCommunicationPortType.from_xml(self, child_element)
        child.parent = obj
        obj.port_list.append(child)
      elif xsi_type.split(":", 1)[ 1 ].lower() == "blackboard":
        child = Blackboard.from_xml(self, child_element)
        child.parent = obj
        obj.port_list.append(child)
      
      elif xsi_type.split(":", 1)[ 1 ].lower() == "buffer":
        child = Buffer.from_xml(self, child_element)
        child.parent = obj
        obj.port_list.append(child)
    return obj
