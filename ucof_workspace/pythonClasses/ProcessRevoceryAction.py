...
# -------------------------------------------------------------------
# Automatically generated class from ecore model!! ProcessRevoceryAction.
# Don't change here, change in source ecore model
# Code generator by Philipp Chrysalidis in 2025
# ======================================================================

class ProcessRevoceryAction(PartitionProcessLevelAction):
  def __init__(self):
    #attributes
    self.id = None
    self.name = None
  ...
  def to_xml(self, reference_name = None):
    ...
    if not reference_name == "healthmanagement" and not reference_name.lower() == "processrevoceryaction":
      element.set("xsi:type", "de.ucof.model.healthmanagement:ProcessRevoceryAction")  
    ...
    if self.errorCode is not None:
      element.set("errorCode", str(self.errorCode))  
    if self.recoveryAction is not None:
      element.set("recoveryAction", str(self.recoveryAction))  
    # Convert children to nested XML elements
    return element

  def from_xml(self, xml_element):
    # Instantiate an object of type ProcessRevoceryAction
    obj = ProcessRevoceryAction()  
    ...
    if "errorCode" in xml_element.attrib:
        obj.errorCode = xml_element.get("errorCode")
    if "recoveryAction" in xml_element.attrib:
        obj.recoveryAction = xml_element.get("recoveryAction")
