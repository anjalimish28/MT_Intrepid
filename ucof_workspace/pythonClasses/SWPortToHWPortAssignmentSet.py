...
from SWPortToHWPortAssignment import SWPortToHWPortAssignment


# -------------------------------------------------------------------
# Automatically generated class from ecore model!! SWPortToHWPortAssignmentSet.
# Don't change here, change in source ecore model
# Code generator by Philipp Chrysalidis in 2025
# ======================================================================

class SWPortToHWPortAssignmentSet(SuperAssignmentSetType):
  def __init__(self):
    #attributes
    self.id = None
    self.name = None
    ...
    self.swPortToHWPortAssignment_list = list()  #Initialize as an empty list for multiple children
    self.parent = None #default is no parent, but can be set for every element
	
    #Check if it's a collection (i.e., upperBound <> 1)
  def add_swPortToHWPortAssignment(self, swPortToHWPortAssignment): #function for adding child
    self.swPortToHWPortAssignment_list.append(swPortToHWPortAssignment)
    swPortToHWPortAssignment.parent = self #link child to parent

  def to_xml(self, reference_name = None):
    element = ET.Element(reference_name)
    if not reference_name == "assignments" and not reference_name.lower() == "swporttohwportassignmentset":
      element.set("xsi:type", "de.ucof.model.assignments:SWPortToHWPortAssignmentSet")  
  ...
  # Multiple children
    for child in self.swPortToHWPortAssignment_list:  
      element.append(child.to_xml("swPortToHWPortAssignment"))  
    return element

  def from_xml(self, xml_element):
    # Instantiate an object of type SWPortToHWPortAssignmentSet
    obj = SWPortToHWPortAssignmentSet()  
    ...
    obj.swPortToHWPortAssignment_list = list()
    for child_element in xml_element.findall("swPortToHWPortAssignment"):
      xsi_type = child_element.get("{http://www.w3.org/2001/XMLSchema-instance}type")
      if not xsi_type:
        child = SWPortToHWPortAssignment.from_xml(self, child_element)
        child.parent = obj
        obj.swPortToHWPortAssignment_list.append(child)
