...
# -------------------------------------------------------------------
# Automatically generated class from ecore model!! SignalRouting.
# Don't change here, change in source ecore model
# Code generator by Philipp Chrysalidis in 2025
# ======================================================================

class SignalRouting(SuperIdentifierType, SuperTrackingType):
  def __init__(self):
    #attributes
    self.id = None
    self.name = None
    ...
    self.version = None
    #children (references)
    self.inSignal = None  #Initialize as a single child (None by default)
    self.outSignal = None  #Initialize as a single child (None by default)
  ...
  def to_xml(self, reference_name = None):
    ...
    if not reference_name == "routing" and not reference_name.lower() == "signalrouting":
      element.set("xsi:type", "de.ucof.model.devices.functions.routing:SignalRouting")  
  ...
  # Single child
    if self.inSignal is not None:
      element.append(self.inSignal.to_xml("inSignal"))  
  # Single child
    if self.outSignal is not None:
      element.append(self.outSignal.to_xml("outSignal"))  
    return element

  def from_xml(self, xml_element):
    # Instantiate an object of type SignalRouting
    obj = SignalRouting()  
    ...
    child_element = xml_element.find("inSignal")
    if child_element is not None:  
      xsi_type = child_element.get("{http://www.w3.org/2001/XMLSchema-instance}type")
      if (not xsi_type) or (xsi_type.split(":", 1)[ 1 ].lower() == "signal"):
        child = Signal.from_xml(self, child_element)
        child.parent = obj
        obj.inSignal = child        
      
    # Reference with a single value: find the child element and set the field if it exists
    child_element = xml_element.find("outSignal")
    if child_element is not None:  
      xsi_type = child_element.get("{http://www.w3.org/2001/XMLSchema-instance}type")
      if (not xsi_type) or (xsi_type.split(":", 1)[ 1 ].lower() == "signal"):
        child = Signal.from_xml(self, child_element)
