...
# -------------------------------------------------------------------
# Automatically generated class from ecore model!! SignalToTargetAssignment.
# Don't change here, change in source ecore model
# Code generator by Philipp Chrysalidis in 2025
# ======================================================================

class SignalToTargetAssignment(SuperAssignmentType):
  def __init__(self):
    #attributes
    self.id = None
    self.name = None
    ...
    self.targetID = None
    self.startBit = None
    self.assignmentTargetType = None
  ...
  def to_xml(self, reference_name = None):
    ...
    if not reference_name == "database" and not reference_name.lower() == "signaltotargetassignment":
      element.set("xsi:type", "de.ucof.model.assignments.database:SignalToTargetAssignment")  
    ...
    if self.startBit is not None:
      element.set("startBit", str(self.startBit))  
    if self.assignmentTargetType is not None:
      element.set("assignmentTargetType", str(self.assignmentTargetType))  
    if self.initialValue is not None:
      element.set("initialValue", str(self.initialValue))  
    # Convert children to nested XML elements
    return element

  def from_xml(self, xml_element):
    # Instantiate an object of type SignalToTargetAssignment
    obj = SignalToTargetAssignment()  
    ...
    if "startBit" in xml_element.attrib:
        obj.startBit = xml_element.get("startBit")
    if "assignmentTargetType" in xml_element.attrib:
        obj.assignmentTargetType = xml_element.get("assignmentTargetType")
    if "initialValue" in xml_element.attrib:
        obj.initialValue = xml_element.get("initialValue")
