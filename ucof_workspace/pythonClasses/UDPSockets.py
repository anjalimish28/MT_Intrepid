...
# -------------------------------------------------------------------
# Automatically generated class from ecore model!! UDPSockets.
# Don't change here, change in source ecore model
# Code generator by Philipp Chrysalidis in 2025
# ======================================================================

class UDPSockets(SuperIdentifierType, SuperTrackingType):
  def __init__(self):
    #attributes
    self.id = None
    self.name = None
    ...
    self.udpSocket_list = list()  #Initialize as an empty list for multiple children
    self.parent = None #default is no parent, but can be set for every element
	
    #Check if it's a collection (i.e., upperBound <> 1)
  def add_udpSocket(self, udpSocket): #function for adding child
    self.udpSocket_list.append(udpSocket)
    udpSocket.parent = self #link child to parent

  def to_xml(self, reference_name = None):
    element = ET.Element(reference_name)
    if not reference_name == "configECIC" and not reference_name.lower() == "udpsockets":
      element.set("xsi:type", "de.ucof.model.communication.configECIC:UDPSockets")  
  ...
  # Multiple children
    for child in self.udpSocket_list:  
      element.append(child.to_xml("udpSocket"))  
    return element

  def from_xml(self, xml_element):
    # Instantiate an object of type UDPSockets
    obj = UDPSockets()  
    ...
    obj.udpSocket_list = list()
    for child_element in xml_element.findall("udpSocket"):
      xsi_type = child_element.get("{http://www.w3.org/2001/XMLSchema-instance}type")
      if not xsi_type:
        child = UDPSocket.from_xml(self, child_element)
        child.parent = obj
        obj.udpSocket_list.append(child)
