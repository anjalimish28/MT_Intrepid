...
from WiredSuper import WiredSuper
from Analogue import Analogue

from Discrete import Discrete

# -------------------------------------------------------------------
# Automatically generated class from ecore model!! WiredInterfaces.
# Don't change here, change in source ecore model
# Code generator by Philipp Chrysalidis in 2025
# ======================================================================

class WiredInterfaces(SuperCommunicationInterfaceType):
  def __init__(self):
    #attributes
    self.id = None
    self.name = None
    ...
    self.wiredInterface_list = list()  #Initialize as an empty list for multiple children
    self.parent = None #default is no parent, but can be set for every element
	
    #Check if it's a collection (i.e., upperBound <> 1)
  def add_wiredInterface(self, wiredInterface): #function for adding child
    self.wiredInterface_list.append(wiredInterface)
    wiredInterface.parent = self #link child to parent

  def to_xml(self, reference_name = None):
    element = ET.Element(reference_name)
    if not reference_name == "basedonwired" and not reference_name.lower() == "wiredinterfaces":
      element.set("xsi:type", "de.ucof.model.communication.basedonwired:WiredInterfaces")  
  ...
  # Multiple children
    for child in self.wiredInterface_list:  
      element.append(child.to_xml("wiredInterface"))  
    return element

  def from_xml(self, xml_element):
    # Instantiate an object of type WiredInterfaces
    obj = WiredInterfaces()  
    ...
    obj.wiredInterface_list = list()
    for child_element in xml_element.findall("wiredInterface"):
      xsi_type = child_element.get("{http://www.w3.org/2001/XMLSchema-instance}type")
      if not xsi_type:
        child = WiredSuper.from_xml(self, child_element)
        child.parent = obj
        obj.wiredInterface_list.append(child)
      elif xsi_type.split(":", 1)[ 1 ].lower() == "analogue":
        child = Analogue.from_xml(self, child_element)
        child.parent = obj
        obj.wiredInterface_list.append(child)
      
      elif xsi_type.split(":", 1)[ 1 ].lower() == "discrete":
        child = Discrete.from_xml(self, child_element)
        child.parent = obj
        obj.wiredInterface_list.append(child)
    return obj
