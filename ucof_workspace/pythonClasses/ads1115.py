...
from adcPorts import adcPorts


# -------------------------------------------------------------------
# Automatically generated class from ecore model!! ads1115.
# Don't change here, change in source ecore model
# Code generator by Philipp Chrysalidis in 2025
# ======================================================================

class ads1115(SuperTargetType, SuperPowerType):
  def __init__(self):
    #attributes
    self.id = None
    self.name = None
  ...
  def to_xml(self, reference_name = None):
    ...
    if not reference_name == "adc" and not reference_name.lower() == "ads1115":
      element.set("xsi:type", "de.ucof.model.targets.adc:ads1115")  
  ...
  def from_xml(self, xml_element):
    # Instantiate an object of type ads1115
    obj = ads1115()  
    ...
    if child_element is not None:  
      xsi_type = child_element.get("{http://www.w3.org/2001/XMLSchema-instance}type")
      if (not xsi_type) or (xsi_type.split(":", 1)[ 1 ].lower() == "adcports"):
        child = adcPorts.from_xml(self, child_element)
        child.parent = obj
        obj.ports = child        
