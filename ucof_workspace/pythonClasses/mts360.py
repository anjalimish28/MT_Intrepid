...
from hallPorts import hallPorts


# -------------------------------------------------------------------
# Automatically generated class from ecore model!! mts360.
# Don't change here, change in source ecore model
# Code generator by Philipp Chrysalidis in 2025
# ======================================================================

class mts360(SuperTargetType, SuperPowerType):
  def __init__(self):
    #attributes
    self.id = None
    self.name = None
  ...
  def to_xml(self, reference_name = None):
    ...
    if not reference_name == "hallsensor" and not reference_name.lower() == "mts360":
      element.set("xsi:type", "de.ucof.model.targets.hallsensor:mts360")  
  ...
  def from_xml(self, xml_element):
    # Instantiate an object of type mts360
    obj = mts360()  
    ...
    if child_element is not None:  
      xsi_type = child_element.get("{http://www.w3.org/2001/XMLSchema-instance}type")
      if (not xsi_type) or (xsi_type.split(":", 1)[ 1 ].lower() == "hallports"):
        child = hallPorts.from_xml(self, child_element)
        child.parent = obj
        obj.ports = child        
